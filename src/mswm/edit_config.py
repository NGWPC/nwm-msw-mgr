...
def create_valid_config_file(
    yaml_file: Path, valid_run_path: Path, valid_config_file: Path, valid_run_name: str
) -> None:
    """
...
def create_valid_realization_file(
    agent: "Agent", eval_params: "EvaluationOptions", params: "pd.DataFrame", valid_run_name: str
) -> None:
    """
    ...
    if config_valid["global"]["forcing"]["provider"] == "ForcingsEngineLumpedDataProvider":
        ...
        config_valid["global"]["forcing"]["params"]["init_config"] = fe_config.with_name(
            fe_config.stem + "_valid" + fe_config.suffix
        )
    ...
    for m in config_valid["global"]["formulations"][0]["params"]["modules"]:
        if m["params"]["model_type_name"] in ["NoahOWP", "UEB"]:
            ...
            m["params"]["init_config"] = os.path.join(
                os.path.dirname(m1), os.path.basename(m1).replace("calib", "valid")
            )
    ...
    logger.info(
        f"valid_output_vars set in realization: {config_valid['global']['formulations'][0]['params']['output_variables']}"
    )
    ...
    #     config_valid['global']['formulations'][0]['params']['output_header_fields16,17c16,17
